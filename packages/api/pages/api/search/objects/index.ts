...
import { ApiSearchRequest, PaginatedApiResponse, StorageObject } from '@core/types/api';
...
import { apiSearchObjects } from '@handlers/apiObjectServices';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseData | ResponseError>) {
  ...
  try {
    ...
    const result = await apiSearchObjects(String(keyword), searchMode as SearchModes, Number(limit), Number(offset));
