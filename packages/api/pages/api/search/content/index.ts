...
import { ApiSearchRequest, PaginatedApiResponse, StorageContent, StorageObject } from '@core/types/api';
...
type ResponseData = PaginatedApiResponse<StorageContent>;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseData | ResponseError>) {
  ...
  try {
    ...
    const result = await apiSearchContent(String(keyword), searchMode as SearchModes, Number(limit), Number(offset));
